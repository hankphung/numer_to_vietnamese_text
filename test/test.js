...
describe('Number to Vietnamese text', function() {
   ...
   it("should beable to handle of billions", function(){
      assert.equal(to_vietnamese(21000000000,'đồng'),"hai mươi mốt tỉ đồng")
   })
   it("should beable to handle even thousand of billions", function(){
      assert.equal(to_vietnamese(210000000000001,'đồng'),"hai trăm mười nghìn tỉ không trăm lẻ một đồng")
   })
   it("should beable to handle thousands of billions", function(){
      assert.equal(to_vietnamese(1230950880055,'đồng'),"một nghìn hai trăm ba mươi tỉ chín trăm năm mươi triệu tám trăm tám mươi nghìn không trăm năm mươi lăm đồng")
   })
})